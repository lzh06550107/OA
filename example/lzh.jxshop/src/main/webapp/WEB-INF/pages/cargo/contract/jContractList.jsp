...
<li id="new"><a href="#" onclick="formSubmit('1/submit.action','_self');this.blur();">上报</a></li>
<li id="new"><a href="#" onclick="formSubmit('0/cancel.action','_self');this.blur();">取消</a></li>
...
<table id="ec_table" class="tableRegion" width="98%" >
	...
	<tr>
		...
		<td class="tableHeader">货物总费用</td>
		<td class="tableHeader">附件总费用</td>
	...
	<tr class="odd" onmouseover="this.className='highlight'" onmouseout="this.className='odd'" >
		<td><input type="checkbox" name="contractId" value="${o.id}"/></td>
		<td>${status.index+1}</td>
		<td>${o.customName}</td>
		<td><a href="toview.action?id=${o.id}">${o.contractNo}</a></td>
		<td>${o.cpnum }/<c:choose><c:when test="${o.epnum==null}">0</c:when><c:otherwise>${o.epnum }</c:otherwise></c:choose></td>		
		<td>${o.inputBy}</td>
		<td>${o.checkBy}</td>
		<td>${o.inspector}</td>
		<td>${o.tradeTerms}</td>
		...
		<td>
			<a href="../contractproduct/tocreate.action?contractId=${o.id}">[货物]</a>
		</td>
	</tr>
	</c:forEach>
