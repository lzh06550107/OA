			...
			<tbody id="TableData" class="dataContainer" datakey="userList">
				<s:iterator value="recordList">
					<tr class="TableDetail1 template">
						...
						<td>${description}&nbsp;</td>
						<td><s:if test="#session.user.hasPrivilegeByName('用户删除')">
								<s:a onclick="return window.confirm('确定删除当前记录吗？')"
									action="user_delete?id=%{id}" namespace="/">删除</s:a>
							</s:if> <s:if test="#session.user.hasPrivilegeByName('用户修改')">
								<s:a action="user_editUI?id=%{id}" namespace="/">修改</s:a>
							</s:if> <s:if test="#session.user.hasPrivilegeByName('用户初始化密码')">
								<s:a action="user_intiPassword?id=%{id}" namespace="/"
